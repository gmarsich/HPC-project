...
day=$(date '+%Y-%m-%d')
time=$(date '+%T')
results_dir="results_THIN_single_${day}_${time}"
mkdir -p "$results_dir"
...
for ((run = 1; run <= num_runs; run++)); do
    ...
    for ((input = start; input <= end; input += step)); do
        ...
        echo "$output_mkl" >>"${results_dir}/${results_dir}_mkl_${run}.txt"
        output_openBLAS=$(srun -n1 --cpus-per-task=12 ./gemm_oblas.x "$input" "$input" "$input" | tail -n 1 | grep ",")
        echo "$output_openBLAS" >>"${results_dir}/${results_dir}_openBLAS_${run}.txt"
    done
done
